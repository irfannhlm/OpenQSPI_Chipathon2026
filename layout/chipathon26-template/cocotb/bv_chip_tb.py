...
def chip_top_runner():
    ...
    if gl:
        ...
        sources.append(proj_path / f"../bv_chip/{top_macro}_chip.v")
        sources.append(proj_path / f"../bv_chip/{top_macro}_padring.prep.v")
        ...
        # top_sdf = proj_path / f"../bv_chip/sdf/{sdf_corner}/{top_macro}_chip__{sdf_corner}.sdf"
    ...
    if sim == "questa":
        ...
        if gl:
            ...
            if len(sdf_paths) > 0 and sdf_paths[0].exists():
                plusargs += ["-sdfmax", f"/bv_chip_tb/i_chip_top/u_core={sdf_paths[0]}"]


    runner = get_runner(sim)
    runner.build(
        sources=sources,
        hdl_toplevel=f"bv_chip_tb",
        defines=defines,
        always=True,
        includes=includes,
