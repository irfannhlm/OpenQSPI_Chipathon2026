...
STARTUP_TIME = 6  # in ms; exceeds the MX25L tVSL power-up gate

@cocotb.test()
async def test_triple_flash_jedec_id(dut):
    """Test Read JEDEC ID across Infineon, Winbond, and Macronix flashes"""
    
    cocotb.start_soon(Clock(dut.clk_i, 1000000000 / CLOCK_FREQ, unit="ns").start())
    ...
    dut.qspi_timeout_i.value = int(CLOCK_FREQ * 1e-3) # 1ms timeout (in cycles)
    ...
    await Timer(STARTUP_TIME, unit="ms")
    
    # Configure QSPI Master for JEDEC ID (0x9F)
    ...
    # Define test targets (Name, CS_BITMASK)
    targets = [
        ("Infineon", 1), # 0b001
        ("Winbond",  2), # 0b010
        ("Macronix", 4)  # 0b100
    ...
    # Execute the transaction for each chip
