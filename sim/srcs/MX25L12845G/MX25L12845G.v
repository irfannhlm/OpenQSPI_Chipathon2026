// *============================================================================================== 
// *
// *   MX25L12845G.v - 128M-BIT CMOS Serial Flash Memory
// *
// *           COPYRIGHT 2017 Macronix International Co., Ltd.
// *----------------------------------------------------------------------------------------------
// * Environment  : Cadence NC-Verilog
// * Reference Doc: MX25L12845G REV.1.5,FEB.08,2017
// * Creation Date: @(#)$Date: 2017/02/27 09:12:07 $
// * Version      : @(#)$Revision: 1.12 $
// * Description  : There is only one module in this file
// *                module MX25L12845G->behavior model for the 128M-Bit flash
// *----------------------------------------------------------------------------------------------
...
// * Note 2:power setup time is tVSL = 1200_000 ns, so after power up, chip can be enable.
...
`define MX25L12845GM 16SOP
//`define MX25L12845GM2 8SOP
...
// *============================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
        `ifdef LOADING_30pf
          `define Vtclqv 8  //30pf:8ns, 15pf:6ns
        `elsif LOADING_15pf
...
// *============================================================================================== 
    ...
    parameter   tSHQZ           = 8,        // CS High to SO Float Time [ns]
                ...
                tQVD            = 0.6,         // Data Output Valid Time Difference among all SIO pins
                ...
                tCE             = 55_000,      // unit is ms instead of ns  
                ...
                tCE_FM          = 45_000,      // unit is ms instead of ns for factory mode
                ...
                tVSL            = 1200_000;     // Time delay to chip select allowed

    parameter   tPGM_CHK        = 2_000,   // 2 us
                tERS_CHK        = 100_000;   // 100 us
    parameter   tESL            = 25_000,       // delay after erase suspend command
                tPSL            = 25_000,       // delay after program suspend command
...
//    time    tRES;
    ...
    wire    ODS_10;
    wire    ODS_11;
    assign ODS_00 = !CR[1] && !CR[0]; 
    assign ODS_01 = !CR[1] && CR[0]; 
    assign ODS_10 = CR[1] && !CR[0]; 
    assign ODS_11 = CR[1] && CR[0]; 
    ...
    begin : memory_initialize
        ...
        SFDP_ARRAY[9'h69] =  8'hbe;
        SFDP_ARRAY[9'h6a] =  8'h29;
        SFDP_ARRAY[9'h6b] =  8'hff;
        SFDP_ARRAY[9'h6c] =  8'hf0;
        SFDP_ARRAY[9'h6d] =  8'hd0;
        SFDP_ARRAY[9'h6e] =  8'hff;
        SFDP_ARRAY[9'h6f] =  8'hff;
        SFDP_ARRAY[9'hc0] =  8'h00;
        SFDP_ARRAY[9'hc1] =  8'h00;
        SFDP_ARRAY[9'hc2] =  8'hff;
        SFDP_ARRAY[9'hc3] =  8'hff;
        SFDP_ARRAY[9'hc4] =  8'hff;
        SFDP_ARRAY[9'hc5] =  8'hff;
        SFDP_ARRAY[9'hc6] =  8'hff;
...
// *============================================================================================== 
    ...
    always @ ( negedge SCLK or posedge CS_INT ) begin
        if ( CS_INT == 1'b0 && DDR_Read_Mode_IN == 1'b1 ) begin
            if ( SI_IN_EN == 1'b1 && SO_IN_EN == 1'b1 && WP_IN_EN == 1'b1 && SIO3_IN_EN == 1'b1 ) begin
                SI_Reg[31:0] = {8'b0, SI_Reg[19:0], SIO3, WP, SO, SI};
            end 
            else  if ( SI_IN_EN == 1'b1 && SO_IN_EN == 1'b1 ) begin
                SI_Reg[31:0] = {8'b0, SI_Reg[21:0], SO, SI};
            end
            else begin 
                SI_Reg[31:0] = {8'b0, SI_Reg[22:0], SI};
            end
            if  (  (ENDDR4XIO_Read_Mode == 1'b1 && Bit == 2 && !ENQUAD) ||
                   ...
                   (DDRRead_4XIO_Mode == 1'b1 && Bit == 19 && ENDDR4XIO_Read_Mode == 1'b0 && ENQUAD)  ) begin 
    ...
    always @ ( posedge SCLK or posedge CS_INT ) begin
        if ( CS_INT == 1'b0 && DDR_Read_Mode_IN == 1'b1 && ENDDR4XIO_Read_Mode == 1'b1 ) begin
            if ( SI_IN_EN == 1'b1 && SO_IN_EN == 1'b1 && WP_IN_EN == 1'b1 && SIO3_IN_EN == 1'b1 ) begin
                SI_Reg_DDR[31:0] = {8'b0, SI_Reg_DDR[19:0], SIO3, WP, SO, SI};
            end
        end
        else begin
...
`endif
                        ...
                        begin
                            if (!DP_Mode && !WIP && WEL && Norm_Array_Mode && WPSEL_Mode && Chip_EN && ~HPM_RD && !EPSUSP && !ENQUAD) begin
                                if ( CS_INT == 1'b1 && Bit == 23 ) begin
                                    //$display( $time, " Enter Write Lock Register Funciton ...");
                                    ->WRLR_Event;
                                    WRLR_Mode = 1'b1;
                                end
    ...
    always @ ( RST_Event ) begin
        ...
        else if ( WR_WPSEL_Mode || WRLR_Mode || WRSPB_Mode || WRSCUR_Mode || PP_4XIO_Mode || PP_1XIO_Mode ) begin
            #(tREADY2_P);
        end
...
// *========================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
    /*  Description: Execute program DPB register                           */
    /*----------------------------------------------------------------------*/
